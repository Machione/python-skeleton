...
- The repository must have "Default to PR title for squash merge commits"
  enabled; otherwise squash merges record branch commit subjects, which
  release-please cannot parse. To record a release after the fact, wrap
  `BEGIN_COMMIT_OVERRIDE` and `END_COMMIT_OVERRIDE` around the intended
  commit message in the merged PR's body and re-run the Release workflow.
...
- `CHANGELOG.md` is generated by release-please and auto-formatted by the
  Release workflow before the release PR merges; never hand-edit it.
