...
distributions are published automatically. The release PR's `CHANGELOG.md` is
generated by release-please and auto-formatted by the Release workflow before
auto-merge. Define the ruleset's required status checks so that auto-merged
release PRs wait for CI.
